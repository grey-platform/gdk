...
int  SpeakNextClause(_FILE *f_text, const void *text_in, int control);
...
void Write4Bytes(_FILE *f, int value);
int Read4Bytes(_FILE *f);
int Reverse4Bytes(int word);
int CompileDictionary(const char *dsource, const char *dict_name, _FILE *log, char *err_name,int flags);
...
int MbrolaTranslate(PHONEME_LIST *plist, int n_phonemes, int resume, _FILE *f_mbrola);
